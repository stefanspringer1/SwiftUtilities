...
public func normalizeLengthNumber(inText text: String?, withDecimalPlacesForUnit: [String:Int], droppingDecimalPlacesWhenZero: Bool = false) -> String? {
    ...
    guard numberParts.count == 2 else { return "\(numberText)\(droppingDecimalPlacesWhenZero ? "" : ".\(String(repeating: "0", count: numberOfDecimalPlaces))")\(unitText)" }
    ...
    if droppingDecimalPlacesWhenZero, decimalPlaces.contains(/^0+$/) {
