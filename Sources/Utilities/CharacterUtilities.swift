...
/*
 Code for combining a list of characters:
 ...
 var ranges = [String]()
 var singles = [String]()
 for n in list.sorted(by: <) {
     if start == -1 {
         start = n
     } else if last > -1, n != last + 1 {
         if start != last {
             ranges.append("0x\(String(format: "%04X", start))...0x\(String(format: "%04X", last))")
         } else {
             singles.append("0x\(String(format: "%04X", n))")
         }
         start = n
     }
     last = n
 ...
 print(singles.joined(separator: ", "))
 ```
 */
...
public enum CharacterClass: CombinedCharacterClass, CaseIterable {
    ...
    case EMPTY                 = 0b0000000000000000000000000000000000
    case CAPITAL_LATIN_LETTERS = 0b0000000000000000000000000000000001
    case SMALL_LATIN_LETTERS   = 0b0000000000000000000000000000000010
    case LATIN_LETTERS         = 0b0000000000000000000000000000000011 // do not insert characters directly into this class, use SMALL_LATIN_LETTERS or CAPITAL_LATIN_LETTERS
    case COMBINING_ABOVE       = 0b0000000000000000000000000000000100
    case COMBINING_BELOW       = 0b0000000000000000000000000000001000
    case COMBINING_MIDDLE      = 0b0000000000000000000000000000010000
    case COMBINING             = 0b0000000000000000000000000000011100 // do not insert characters directly into this class, use COMBINING_ABOVE, COMBINING_BELOW, or COMBINING_MIDDLE
    //case ISO_8859_1            = 0b0000000000000000000000000000100000
    case CAPITAL_GREEK_LETTERS = 0b0000000000000000000000000001000000
    case SMALL_GREEK_LETTERS   = 0b0000000000000000000000000010000000
    case GREEK_LETTERS         = 0b0000000000000000000000000011000000 // do not insert characters directly into this class, use SMALL_GREEK_LETTERS or CAPITAL_GREEK_LETTERS
    case MATHEMATICAL          = 0b0000000000000000000000000100000000
    case ROMAN                 = 0b0000000000000000000000001000000000
    case ITALIC                = 0b0000000000000000000000010000000000
    case BOLD                  = 0b0000000000000000000000100000000000
    case SCRIPT                = 0b0000000000000000000001000000000000
    case GOTHIC                = 0b0000000000000000000010000000000000
    case SANSSERIF             = 0b0000000000000000000100000000000000
    case MONOSPACE             = 0b0000000000000000001000000000000000
    case DOUBLE_STRUCK         = 0b0000000000000000010000000000000000
    case DIGIT                 = 0b0000000000000000100000000000000000
    case LARGE_OPERATORS       = 0b0000000000000001000000000000000000
    case BINARY_OPERATIONS     = 0b0000000000000010000000000000000000
    case RELATIONS             = 0b0000000000000100000000000000000000
    case NEGATED_RELATIONS     = 0b0000000000001000000000000000000000
    case ARROWS                = 0b0000000000010000000000000000000000
    case OPENING_DELIMITERS    = 0b0000000000100000000000000000000000
    case CLOSING_DELIMITERS    = 0b0000000001000000000000000000000000
    case PUNCTUATION           = 0b0000000010000000000000000000000000
    case LEFT_RIGHT_ARROWS     = 0b0000000100000000000000000000000000
    case CYRILLIC              = 0b0000001000000000000000000000000000
    case ACCENTED              = 0b0000010000000000000000000000000000
    case TRIVIAL_SPACES        = 0b0000100000000000000000000000000000
    case NONTRIVIAL_SPACES     = 0b0001000000000000000000000000000000
    case SPACES                = 0b0001100000000000000000000000000000 // do not insert characters directly into this class, use TRIVIAL_SPACES or NONTRIVIAL_SPACES
    case POSTFIX_OPERATORS     = 0b0010000000000000000000000000000000
    case LETTERS               = 0b0100000000000000000000000000000000
    case ARBORTEXT_SPACE_BUG   = 0b1000000000000000000000000000000000 // the Arbortext editor falsely sets a space between those characters and a combining character
    case ALL                   = 0b1111111111111111111111111111111111
...
public class CharacterClasses {
    ...
    public init() {
        ...
        let combiningAbove = UCCodePoints(
            // ---- ranges:
            [
                0x0300...0x0315, 0x031A...0x031B, 0x033D...0x0344,
...
//        }
        ...
        do {
            let codePoints = UCCodePoints(
                ...
                [
                    ...
                    0x00F8...0x026E, 0x0370...0x0372, 0x0372...0x0376, 0x0376...0x0377,
                    0x037A...0x037F, 0x0384...0x0386, 0x0386...0x0388, 0x0388...0x0389,
                    0x0389...0x038A, 0x038E...0x038F, 0x038F...0x0391, 0x0391...0x0392,
                    0x0392...0x0393, 0x0393...0x0394, 0x0394...0x0395, 0x0395...0x0396,
                    0x0396...0x0397, 0x0397...0x0398, 0x0398...0x0399, 0x0399...0x039A,
                    0x039A...0x039B, 0x039B...0x039C, 0x039C...0x039D, 0x039D...0x039E,
                    0x039E...0x039F, 0x039F...0x03A0, 0x03A0...0x03A1, 0x03A3...0x0482,
            ...
            codePoints.forEach {
                self.add(.LETTERS, toCodePoint: $0)
            }
            
            self.codePoints[.LETTERS] = codePoints
        }
        ...
        do {
            let codePoints = UCCodePoints(
                ...
                // ---- single codepoints:
                [
                    0x0028, 0x005B, 0x007B, 0x007C, 0x0028, 0x002F, 0x2016, 0x2191, 0x2193, 0x2195,
                    0x21D1, 0x21D3, 0x21D5, 0x2308, 0x230A, 0x23B0, 0x27E8, 0x2985, 0x300A, 0x3014,
                    0x301A,
                ]
            )
        ...
        do {
            let codePoints = UCCodePoints(
                ...
                // ---- single codepoints:
                [
                    0x0029, 0x005D, 0x007C, 0x007D, 0x002F, 0x2016, 0x2016, 0x2191, 0x2193, 0x2195, 0x21D1,
                    0x21D3, 0x21D5, 0x2309, 0x230B, 0x23B1, 0x27E9, 0x2986, 0x300B, 0x3015, 0x301B,
                ]
            )
...
fileprivate let nontrivialSpacesLowerBound = UnicodeScalar(0x2000)!
fileprivate let nontrivialSpacesUpperBound = UnicodeScalar(0x200B)!

fileprivate let letterlikeSymbolsLowerBound = UnicodeScalar(0x2100)!
fileprivate let letterlikeSymbolsUpperBound = UnicodeScalar(0x214F)!
...
//                if suppressRegexRangeNotation {
                if lowerBound.isCombining(usingCharacterClasses: characterClasses) || upperBound.isCombining(usingCharacterClasses: characterClasses) ||
                    (nontrivialSpacesLowerBound <= lowerBound && lowerBound <= nontrivialSpacesUpperBound) || (nontrivialSpacesLowerBound <= upperBound && upperBound <= nontrivialSpacesUpperBound) ||
                    (letterlikeSymbolsLowerBound <= lowerBound && lowerBound <= letterlikeSymbolsUpperBound) || (letterlikeSymbolsLowerBound <= upperBound && upperBound <= letterlikeSymbolsUpperBound) {
